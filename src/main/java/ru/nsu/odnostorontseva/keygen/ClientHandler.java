...
public class ClientHandler {
    ...
    private final Map<String, CompletableFuture<byte[]>> keyMap;
    ...
    private void submitGeneration(SelectionKey key, ClientContext ctx) {
        ...
        var future = keyMap.computeIfAbsent(ctx.clientName, name -> {
            var f = new CompletableFuture<byte[]>();
            taskQueue.add(new KeyGen(name, f));
            return f;
        });
